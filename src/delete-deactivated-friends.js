(() => {
  ...
  const shouldDeleteDeactivatedFriends = confirm(
    ['Are you sure you want to delete next friends?', deactivatedFriendsNames.join(', ')].join('\n')
  );
  ...
  const deleteButtons = deactivatedFriendsElements.map((el) => {
    ...
    const deleteButton = menuItems.find(
      (menuItem) => menuItem.textContent === 'Удалить из друзей' || menuItem.textContent === 'Unfriend'
    );
    return deleteButton;
  });
